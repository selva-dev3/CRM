...
6. Keep the lint gate clean while editing: remove genuinely unused imports,

state, handlers, query results, and components. Do not leave placeholder

imports, malformed import specifiers, or dead declarations after a cleanup.

7. For `catch` blocks and external data, use `unknown` and narrow it before

reading properties. Reuse shared DTOs and error helpers instead of restoring

`any` to get a page compiling.

8. Before reporting frontend work complete, run `npm run lint` and

`npx tsc --noEmit`; both must pass with zero lint errors and warnings. Run

the relevant tests as well, and inspect the changed diff for accidental

syntax/import damage.
...
## 35. Cross-Layer Data and Contract Correctness

These rules apply whenever frontend work consumes or changes backend data.

1. Trace every affected field through the complete path before changing it:
   `ORM model -> repository -> service -> Pydantic schema -> API client -> UI`.
   Inspect the canonical model and use its real field names; do not use
   `getattr()` fallbacks for imagined or obsolete aliases.
2. Keep query semantics, response names, and UI copy aligned. Data sorted as
   "recently updated" must expose `updated_at`, not `created_at` under an
   `updated_at` key. A foreign-key UUID must be resolved to a display name
   before the UI labels it as a person or organization.
3. Every structured FastAPI request body must use a typed Pydantic model;
   never persist `dict[str, Any]` or `list[dict[str, Any]]` payloads. Every
   endpoint consumed by the frontend must declare a typed `response_model`.
4. Dashboard, report, KPI, and chart values must come from persisted data or
   explicit configuration. Never expose arbitrary percentage splits,
   hardcoded targets, sample metrics, or plausible fake fallbacks. If a
   metric is unavailable, return an explicit unavailable/not-implemented
   state and hide or disable its consumer.
5. Review aggregate endpoints for query count. Do not execute a query inside
   a loop over grouped results; use one grouped query with conditional
   aggregation unless a measured exception is documented and approved.
6. Saved preferences must control the behavior they describe. Do not expose
   configuration for widgets or features that are not rendered or applied.
7. Clickable records must navigate to the specific record route. Prefer a
   semantic `Link`; otherwise provide keyboard activation, visible focus,
   and an accessible name without breaking table semantics. Every icon-only
   or dismiss button requires `type="button"`, an `aria-label`, and visible
   keyboard focus styling.
8. Regression tests must cover populated model mappings and frontend
   loading, error, empty, and populated states. Tests must assert canonical
   names, timestamps, display values, organization scope, specific-record
   navigation, and dismiss-control accessibility where applicable.
9. Do not claim API or UI correctness solely because a branch exists in the
   code. Run the relevant backend tests, frontend tests, lint/type checks,
   and production build, and verify the actual rendered behavior when a
   browser is available.
**

Additional Senior Frontend Engineering Rules — Production Quality

The existing sections already cover architecture, state management, forms, responsiveness, performance, accessibility, security, API contracts, E2E, CI/CD, reuse-first development, and analysis-first implementation. The following rules add only the remaining cross-cutting safeguards.

1. Interaction State & Duplicate-Action Safety

Every interactive control must have a deterministic state model. Account for idle, loading, success, error, disabled, and empty states where applicable.

Prevent duplicate user actions during in-flight operations. Disable or otherwise guard submit, save, delete, approve, create, and similar actions while the same mutation is running.

Preserve user input during recoverable failures. A failed request should not unnecessarily clear forms, filters, selections, or other user-entered state.

Use confirmation for destructive or difficult-to-reverse actions. Clearly identify the affected resource and consequence before execution.

Never use optimistic UI without a rollback or reconciliation strategy. If the server rejects a mutation, restore or reconcile the UI with authoritative server state.

2. Navigation & History Safety

Verify affected navigation flows end-to-end. Test direct navigation, refresh, browser back, browser forward, deep links, and invalid or stale route parameters.

Choose push versus replace intentionally. Do not unexpectedly destroy useful browser history.

After navigation-triggering mutations, verify the destination and source state. Ensure stale selections, dialogs, filters, and query data do not survive incorrectly.

3. Data Integrity & Concurrency

Protect against request race conditions. When multiple requests can overlap, stale responses must not overwrite newer user state.

Keep one authoritative source for each server entity. Avoid maintaining unsynchronized copies of the same server data in React state, Zustand, and Query cache.

Transform API data at a clear boundary. Do not scatter response-shape conversions across presentation components.

Handle partial or missing API fields defensively. Optional backend data must not be assumed to exist.

Use stable entity identity. Never identify records using display text, array position, or mutable values.

After create/update/delete, verify the final visible state against authoritative server data. Cache invalidation or a successful HTTP response alone does not prove the UI is correct.

4. Tables, Lists & Complex Screens

Define explicit behavior for zero, one, many, and paginated records. Verify sorting, filtering, selection, pagination, refresh, and loading behavior where they interact.

Preserve table state intentionally across navigation and refresh. Do not unexpectedly reset page, sort, filter, or selection.

For dependent controls, prevent invalid combinations. When one selection changes another control's valid options, reconcile dependent state deliberately.

For large datasets, use pagination or virtualization based on actual data volume. Do not render large collections unnecessarily.

5. Focus & Accessibility Behavior

Manage focus after important dynamic UI changes. After validation failures, dialogs, sheets, navigation, and meaningful content updates, preserve or move focus intentionally.

Do not block the entire page for localized work. Prefer local loading/error states when only one section is loading or failing.

Interactive overlays must have predictable keyboard dismissal and focus restoration. Do not break shadcn's built-in accessibility behavior.

6. Resource & Runtime Safety

Clean up external resources. Timers, event listeners, observers, subscriptions, and polling must stop when no longer needed.

Polling must have explicit lifecycle and timeout behavior. Stop polling after success, terminal failure, timeout, unmount, or cancellation as appropriate.

Avoid unnecessary client-side JavaScript. Keep static and server-renderable UI on the server and move only genuinely interactive behavior to the client.

Avoid sequential requests when independent requests can safely run in parallel. Do not parallelize requests that have real data dependencies.

7. Runtime Validation & Failure Isolation

Validate every external trust boundary at runtime. API responses, URL parameters, browser storage, and third-party integration data must be validated before use.

Use error boundaries for independently failing UI regions when the page architecture benefits from them. One broken widget should not unnecessarily take down the entire page.

Never expose raw internal errors to users. Show safe, actionable messages while retaining useful diagnostics for developers.

Differentiate error classes when recovery differs. Authentication, authorization, validation, network, server, and unexpected errors should lead to appropriate user actions.

8. Production Verification Gate

Verify the complete user journey, not only the changed component. Start from the user's entry point and verify navigation, API behavior, state transitions, and final UI.

Test both success and failure paths for meaningful mutations.

For shared component or utility changes, verify important existing consumers.

For timing-, viewport-, browser-, or asynchronous-state-dependent behavior, test the relevant boundary conditions.

Do not report a feature or bug fix as complete until actual user-visible behavior has been verified in addition to lint/build/test checks.

Final Production Review Gate

Before reporting work as complete, confirm:

Requirements and affected flows were understood.

Existing components, hooks, queries, utilities, and conventions were reused where appropriate.

No unrelated behavior changed.

Loading, success, empty, error, and disabled states were considered.

Duplicate actions and race conditions were considered where applicable.

Navigation, refresh, back/forward, and deep-link behavior were verified when affected.

Server state and client state remain consistent.

Focus and keyboard behavior were verified for interactive UI.

Timers, listeners, observers, subscriptions, and polling are cleaned up.

Relevant lint/build/test/E2E checks were run.

Actual user-visible behavior was verified before claiming completion.
...
- [ ] Cross-layer fields verified against the ORM model; request and response
      schemas are typed (§35)
- [ ] Metrics use real data, aggregate queries avoid N+1 calls, and loading/
      error/empty/populated states are tested (§35)
