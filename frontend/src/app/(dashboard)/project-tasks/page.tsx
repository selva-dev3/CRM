...
export default function ProjectTasksPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      setTitle(''); setError(''); setIsCreateOpen(false);
  ...
  return <div className="space-y-6 pb-12">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold text-slate-900"><CheckSquare className="text-indigo-600" />Project Tasks</h1><p className="mt-1 text-sm text-slate-500">Plan and track work using the existing CRM task workflow.</p></div>{hasPermission(PERMISSIONS.TASKS.CREATE) && <button type="button" onClick={() => { setError(''); setIsCreateOpen(true); }} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white"><Plus size={16} />Create task</button>}</header>
    ...
    <ModalShell isOpen={isCreateOpen} onClose={() => !createTask.isPending && setIsCreateOpen(false)} title="Create project task" footer={<><button type="button" className="rounded-lg border px-4 py-2 text-sm font-semibold" disabled={createTask.isPending} onClick={() => setIsCreateOpen(false)}>Cancel</button><button type="submit" form="project-task-create-form" className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50" disabled={createTask.isPending}>{createTask.isPending ? 'Creating…' : 'Create task'}</button></>}>
      <form id="project-task-create-form" onSubmit={submit} className="space-y-4 py-4"><div><label htmlFor="project-task-project" className="mb-1 block text-sm font-medium">Project</label><select id="project-task-project" required className="h-10 w-full rounded-lg border px-3" value={projectId} onChange={(event) => { setProjectId(event.target.value); setPage(1); }}><option value="">Select project</option>{selectedProjectIsHidden && <option value={selectedProject.data!.id}>{selectedProject.data!.name}</option>}{projectOptions.map((project) => <option key={project.id} value={project.id}>{project.name}</option>)}</select></div><div><label htmlFor="project-task-title" className="mb-1 block text-sm font-medium">Task title</label><input id="project-task-title" autoFocus required className="h-10 w-full rounded-lg border px-3" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="New project task" /></div>{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}</form>
    </ModalShell>
  </div>;
}
