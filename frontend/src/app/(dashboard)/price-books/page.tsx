...
export default function PriceBooksPage() {
  ...
  async function addBook(event: React.FormEvent) {
    ...
    await runAction(async () => {
      ...
      setSelected(book.id); setName(''); setCurrency('USD'); setIsCreateBookOpen(false);
  ...
  async function addEntry(event: React.FormEvent) {
    ...
    await runAction(async () => {
      ...
      setProductId(''); setPrice(''); setProductSearch(''); setIsAddPriceOpen(false);
    }, 'Could not save product price.');
  }

  return <div className="space-y-6 pb-12">
    <header className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="flex items-center gap-2 text-2xl font-bold"><BookOpen className="text-indigo-600" />Price Books</h1><p className="mt-1 text-sm text-slate-500">Maintain reusable product pricing by currency.</p></div>{hasPermission(PERMISSIONS.PRODUCTS.CREATE) && <Button onClick={() => { setName(''); setCurrency('USD'); setError(''); setIsCreateBookOpen(true); }}><Plus />Create price book</Button>}</header>
    ...
    <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
      ...
      <section className="rounded-xl border bg-white p-4"><div className="mb-4 flex items-center justify-between gap-3"><h2 className="font-bold">Product prices</h2>{activeBookId && hasPermission(PERMISSIONS.PRODUCTS.UPDATE) && <Button size="sm" onClick={() => { setProductId(''); setProductSearch(''); setPrice(''); setError(''); setIsAddPriceOpen(true); }}><Plus />Add product price</Button>}</div><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="bg-slate-50 text-left text-xs uppercase text-slate-500"><tr><th className="p-3">Product</th><th className="p-3">SKU</th><th className="p-3">Unit price</th><th className="p-3">Actions</th></tr></thead><tbody>{entries.data?.map((entry) => <tr key={entry.id} className="border-t"><td className="p-3 font-semibold">{entry.product_name}</td><td className="p-3">{entry.product_sku}</td><td className="p-3">{entry.unit_price.toFixed(2)}</td><td className="p-3">{hasPermission(PERMISSIONS.PRODUCTS.UPDATE) && <button disabled={removeEntry.isPending} aria-label={`Remove ${entry.product_name}`} className="text-rose-600 disabled:opacity-40" onClick={() => runAction(() => removeEntry.mutateAsync({ bookId: activeBookId, productId: entry.product_id }), 'Could not remove product price.')}><Trash2 size={16}/></button>}</td></tr>)}</tbody></table>{entries.isLoading && <p className="p-8 text-center text-slate-500">Loading product prices…</p>}{activeBookId && !entries.isLoading && !entries.isError && !entries.data?.length && <p className="p-8 text-center text-slate-500">No product prices configured.</p>}</div></section>
    </div>
    <ModalShell isOpen={isCreateBookOpen} onClose={() => !create.isPending && setIsCreateBookOpen(false)} title="Create price book" footer={<><Button type="button" variant="outline" disabled={create.isPending} onClick={() => setIsCreateBookOpen(false)}>Cancel</Button><Button form="price-book-create-form" disabled={create.isPending}>{create.isPending ? 'Creating…' : 'Create price book'}</Button></>}><form id="price-book-create-form" onSubmit={addBook} className="space-y-4 py-4"><div><label htmlFor="price-book-name" className="mb-1 block text-sm font-medium">Name</label><input id="price-book-name" autoFocus required className="h-10 w-full rounded-lg border px-3" value={name} onChange={(event) => setName(event.target.value)} /></div><div><label htmlFor="price-book-currency" className="mb-1 block text-sm font-medium">Currency</label><input id="price-book-currency" required maxLength={3} minLength={3} className="h-10 w-full rounded-lg border px-3 uppercase" value={currency} onChange={(event) => setCurrency(event.target.value.toUpperCase())} /></div>{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}</form></ModalShell>
    <ModalShell isOpen={isAddPriceOpen} onClose={() => !upsert.isPending && setIsAddPriceOpen(false)} title="Add product price" footer={<><Button type="button" variant="outline" disabled={upsert.isPending} onClick={() => setIsAddPriceOpen(false)}>Cancel</Button><Button form="product-price-form" disabled={upsert.isPending}>{upsert.isPending ? 'Saving…' : 'Save price'}</Button></>}><form id="product-price-form" onSubmit={addEntry} className="space-y-4 py-4"><div><label htmlFor="product-price-search" className="mb-1 block text-sm font-medium">Search products</label><input id="product-price-search" autoFocus className="h-10 w-full rounded-lg border px-3" placeholder="Search products…" value={productSearch} onChange={(event) => { setProductSearch(event.target.value); setProductId(''); }} /></div><div><label htmlFor="product-price-product" className="mb-1 block text-sm font-medium">Product</label><select id="product-price-product" required className="h-10 w-full rounded-lg border px-3" value={productId} onChange={(event) => setProductId(event.target.value)}><option value="">Select product</option>{products.data?.map((product) => <option key={product.id} value={product.id}>{product.name} ({product.sku})</option>)}</select></div><div><label htmlFor="product-price-value" className="mb-1 block text-sm font-medium">Unit price</label><input id="product-price-value" required min="0" step="0.01" type="number" className="h-10 w-full rounded-lg border px-3" value={price} onChange={(event) => setPrice(event.target.value)} /></div>{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}</form></ModalShell>
  </div>;
}
