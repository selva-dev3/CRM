...
import { AlertTriangle, ArrowLeft, BookOpenCheck, MessageSquareText, Paperclip, Plus, Send, Trash2 } from 'lucide-react';
...
export default function TicketDetailPage() {
  ...
  async function submitComment(event: FormEvent) {
    ...
    try {
      ...
      setComment(''); setError(''); setIsCommentOpen(false);
    } catch (reason) {
  ...
  async function submitArticle(event: FormEvent) {
    ...
    try {
      ...
      setArticleId(''); setError(''); setIsArticleOpen(false);
    } catch (reason) {
  ...
  return <div className="space-y-6 pb-12">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <section className="rounded-xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-bold"><MessageSquareText className="size-5 text-indigo-600" />Conversation</h2><PermissionGate permission={PERMISSIONS.TICKETS.UPDATE}><Button size="sm" onClick={() => { setComment(''); setIsInternal(true); setIsCommentOpen(true); }}><Send className="size-4" />Add comment</Button></PermissionGate></div><div className="my-4 space-y-3">{comments.isLoading && <p className="text-sm text-slate-500">Loading comments…</p>}{!comments.isLoading && !comments.data?.items.length && <p className="text-sm text-slate-500">No comments yet.</p>}{comments.data?.items.map((item) => <article key={item.id} className="rounded-lg bg-slate-50 p-3"><div className="flex justify-between gap-3 text-xs text-slate-500"><span>{item.is_internal ? 'Internal note' : 'Customer-visible reply'}</span><time>{new Date(item.created_at).toLocaleString()}</time></div><p className="mt-2 whitespace-pre-wrap text-sm">{item.body}</p></article>)}</div><PageNavigator page={commentPage} total={comments.data?.total ?? 0} limit={20} onChange={setCommentPage} /></section>
      {canReadKnowledgeBase && <section className="rounded-xl border bg-white p-5"><div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-bold"><BookOpenCheck className="size-5 text-indigo-600" />Knowledge articles</h2><PermissionGate permission={PERMISSIONS.TICKETS.UPDATE}><Button size="sm" onClick={() => { setArticleId(''); setIsArticleOpen(true); }}><Plus className="size-4" />Link article</Button></PermissionGate></div><div className="my-4 space-y-2">{linkedArticles.isLoading && <p className="text-sm text-slate-500">Loading articles…</p>}{!linkedArticles.isLoading && !linkedArticles.data?.length && <p className="text-sm text-slate-500">No knowledge articles linked.</p>}{linkedArticles.data?.map((article) => <div key={article.id} className="flex items-center justify-between gap-3 rounded-lg border p-3"><div><p className="font-medium">{article.title}</p><p className="text-xs text-slate-500">{article.category || 'Uncategorized'}</p></div><PermissionGate permission={PERMISSIONS.TICKETS.UPDATE}><Button variant="ghost" size="icon" aria-label={`Unlink ${article.title}`} disabled={unlinkArticle.isPending} onClick={() => void unlink(article.id)}><Trash2 className="size-4 text-rose-600" /></Button></PermissionGate></div>)}</div></section>}
    </div>
    ...
    <ModalShell isOpen={isCommentOpen} onClose={() => !addComment.isPending && setIsCommentOpen(false)} title="Add ticket comment" footer={<><Button type="button" variant="outline" disabled={addComment.isPending} onClick={() => setIsCommentOpen(false)}>Cancel</Button><Button form="ticket-comment-form" disabled={!comment.trim() || addComment.isPending}>{addComment.isPending ? 'Adding…' : 'Add comment'}</Button></>}><form id="ticket-comment-form" onSubmit={submitComment} className="space-y-4 py-4"><div><label htmlFor="ticket-comment" className="mb-1 block text-sm font-medium">Comment</label><textarea id="ticket-comment" autoFocus required className="min-h-32 w-full rounded-md border p-3 text-sm" value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Add a response or internal note" /></div><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={isInternal} onChange={(event) => setIsInternal(event.target.checked)} />Internal note</label>{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}</form></ModalShell>
    <ModalShell isOpen={isArticleOpen} onClose={() => !linkArticle.isPending && setIsArticleOpen(false)} title="Link knowledge article" footer={<><Button type="button" variant="outline" disabled={linkArticle.isPending} onClick={() => setIsArticleOpen(false)}>Cancel</Button><Button form="ticket-article-form" disabled={!articleId || linkArticle.isPending}>{linkArticle.isPending ? 'Linking…' : 'Link article'}</Button></>}><form id="ticket-article-form" onSubmit={submitArticle} className="space-y-4 py-4"><label htmlFor="ticket-article" className="mb-1 block text-sm font-medium">Published article</label><select id="ticket-article" autoFocus required className="h-10 w-full rounded-md border px-3" value={articleId} onChange={(event) => setArticleId(event.target.value)}><option value="">Select a published article</option>{articles.data?.items.filter((article) => !linkedArticles.data?.some((linked) => linked.id === article.id)).map((article) => <option key={article.id} value={article.id}>{article.title}</option>)}</select>{error && <p role="alert" className="text-sm text-rose-700">{error}</p>}</form></ModalShell>
  </div>;
}
