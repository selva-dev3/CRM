...
function formatAmount(payment: PaymentItem): string {
  return new Intl.NumberFormat(undefined, { style: 'currency', currency: payment.currency || 'USD' }).format(Number(payment.amount || 0));
}
...
function statusLabel(status: string): string {
  return status.toLowerCase() === 'succeeded' ? 'Recorded' : status;
}
...
export default function PaymentsPage() {
  ...
  const columns = useMemo<DataTableColumn<PaymentItem>[]>(() => [
    ...
    { id: 'status', header: 'Status', cell: (item) => <span className={`rounded-full border px-2.5 py-1 text-xs font-semibold ${statusClass(item.status)}`}>{statusLabel(item.status)}</span> },
    { id: 'paid_at', header: 'Paid date', cell: (item) => <span className="text-sm text-slate-600">{item.payment_date || formatDate(item.paid_at || item.created_at)}</span> },
  ...
  return (
    <div className="w-full space-y-6 pb-12">
      ...
      <DataTable<PaymentItem>
        ...
        emptyDescription={query.isLoading ? 'Fetching payment records.' : 'Add payments from an accepted invoice.'}
        searchValue={search}
