...
export interface Ticket { id: string; ticket_number: string; subject: string; description: string; status: string; priority: string; source: string; contact_id: string | null; company_id: string | null; assigned_to: string | null; team_id: string | null; sla_policy_id: string | null; first_response_due_at: string | null; resolution_due_at: string | null; resolved_at: string | null; closed_at: string | null; escalated_at: string | null; escalation_reason: string | null; created_at: string; updated_at: string }
export interface TicketComment { id:string; user_id:string|null; body:string; is_internal:boolean; created_at:string }
export interface TicketAttachment { id:string; filename:string; file_size:number; mime_type:string; download_url:string; uploaded_at:string }
...
export function useTicketComments(id:string,page=1){return useQuery({queryKey:['tickets',id,'comments',page],queryFn:()=>paged<TicketComment>(`/tickets/${id}/comments?page=${page}&limit=20`),enabled:Boolean(id)});}
export function useTicketAttachments(id:string,page=1,enabled=true){return useQuery({queryKey:['tickets',id,'attachments',page],queryFn:()=>paged<TicketAttachment>(`/tickets/${id}/attachments?page=${page}&limit=20`),enabled:Boolean(id)&&enabled});}
...
export function useUpdateTicket() { const client=useQueryClient(); return useMutation({mutationFn:({id,payload}:{id:string;payload:Record<string,unknown>})=>apiClient.patch<Ticket>(`/tickets/${id}`,payload),onSuccess:(item)=>{client.setQueryData(['tickets',item.id],item);client.invalidateQueries({queryKey:['tickets']});}}); }
...
export function useEscalateTicket(){const client=useQueryClient();return useMutation({mutationFn:({ticketId,reason,assignedTo}:{ticketId:string;reason:string;assignedTo?:string})=>apiClient.post<Ticket>(`/tickets/${ticketId}/escalate`,{reason,assigned_to:assignedTo||null}),onSuccess:(item)=>{client.setQueryData(['tickets',item.id],item);client.invalidateQueries({queryKey:['tickets']});}});}
export function useUploadTicketAttachment(){const client=useQueryClient();return useMutation({mutationFn:({ticketId,file}:{ticketId:string;file:File})=>{const body=new FormData();body.append('file',file);return apiClient.post<TicketAttachment>(`/tickets/${ticketId}/attachments`,body);},onSuccess:(_data,variables)=>client.invalidateQueries({queryKey:['tickets',variables.ticketId,'attachments']})});}
