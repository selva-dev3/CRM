...
export function WhatsAppInbox() {
  ...
  useEffect(() => {
    ...
    const newestInbound = [...messages.data.items]
      .reverse()
  ...
  return <div className="grid min-h-[65vh] grid-cols-1 gap-4 lg:h-[calc(100dvh-15rem)] lg:min-h-0 lg:grid-cols-[22rem_1fr]">
    <Card className="flex min-h-0 flex-col overflow-hidden">
      <div className="shrink-0 border-b p-4"><Input aria-label="Search conversations" placeholder="Search customer phone" value={search} onChange={(event) => { setSearch(event.target.value); setConversationOffset(0); }} /></div>
      <div className="min-h-0 flex-1 overflow-y-auto">
        {list.isLoading ? <Loader2 className="m-6 size-5 animate-spin" aria-label="Loading conversations" /> : list.isError ? <p className="p-4 text-sm text-destructive" role="alert">{getErrorMessage(list.error, 'Unable to load conversations.')}</p> : !list.data?.items.length ? <p className="p-4 text-sm text-muted-foreground">No conversations found.</p> : <ul>{list.data.items.map((item) => <li key={item.id}><button type="button" onClick={() => open(item)} className="flex min-h-20 w-full items-center justify-between border-b p-4 text-left hover:bg-muted focus-visible:outline-none focus-visible:ring-2"><span><span className="block font-medium">{item.customer_phone}</span><span className="text-xs text-muted-foreground">{item.identity_state.replaceAll('_', ' ')} · {item.ai_enabled ? 'AI' : 'Human'}</span></span>{item.unread_count > 0 && <span className="rounded-full bg-primary px-2 py-1 text-xs text-primary-foreground">{item.unread_count}</span>}</button></li>)}</ul>}
      </div>
      {!list.isLoading && !list.isError && (list.data?.total ?? 0) > 50 && <div className="flex shrink-0 items-center justify-between border-t p-3"><Button type="button" size="sm" variant="outline" disabled={conversationOffset === 0} onClick={() => setConversationOffset((value) => Math.max(0, value - 50))}>Previous</Button><span className="text-xs text-muted-foreground">{conversationOffset + 1}–{Math.min(conversationOffset + 50, list.data?.total ?? 0)} of {list.data?.total ?? 0}</span><Button type="button" size="sm" variant="outline" disabled={conversationOffset + 50 >= (list.data?.total ?? 0)} onClick={() => setConversationOffset((value) => value + 50)}>Next</Button></div>}
    </Card>
    <Card className="flex min-h-0 flex-col overflow-hidden">
