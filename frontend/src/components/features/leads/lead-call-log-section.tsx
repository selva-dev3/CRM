    ...
    <Card className="space-y-5 rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
      ...
      >
        <form noValidate onSubmit={submitCall} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-1.5"><Label htmlFor="call-timestamp">Call Date / Time *</Label><Input id="call-timestamp" type="datetime-local" {...form.register('timestamp')} /><p className="text-xs text-rose-600">{form.formState.errors.timestamp?.message}</p></div>
          <div className="space-y-1.5"><Label htmlFor="call-duration">Duration (seconds) *</Label><Input id="call-duration" type="number" min={0} max={86400} step={1} {...form.register('duration_seconds', { valueAsNumber: true })} /><p className="text-xs text-rose-600">{form.formState.errors.duration_seconds?.message}</p></div>
          ...
          {followUpRequired && <div className="space-y-1.5"><Label htmlFor="call-follow-up">Follow-up Date / Time *</Label><Input id="call-follow-up" type="datetime-local" {...form.register('follow_up_at')} /><p className="text-xs text-rose-600">{form.formState.errors.follow_up_at?.message}</p></div>}
