...
def stripe_minor_units(amount: object, currency: str) -> int:
    ...
    if currency.upper() not in {"INR", "USD", "EUR", "GBP", "CAD", "AUD", "SGD"}:
        raise APIException(
            message="Stripe minor-unit rules for this currency are not configured",
...
class InvoicePaymentService:
    ...
    def _client(self):
        if not settings.STRIPE_SECRET_KEY:
            raise APIException(
                message="Stripe is not configured", code="PAYMENT_NOT_CONFIGURED", status_code=503
            )
        ...
        try:
            ...
            if invoice.stripe_checkout_session_id:
                session = await asyncio.to_thread(
                    client.v1.checkout.sessions.retrieve, invoice.stripe_checkout_session_id
                )
                ...
                if session.status != "expired":
                    raise ConflictError(
                        message="Checkout is completed or expired; reconcile payment before issuing another session"
                    )
                generation += 1
            ...
            if not session.id or not session.url:
                raise APIException(
                    message="Stripe returned an incomplete checkout", status_code=502
                )
            await self.repository.save_checkout(
                db, invoice, session_id=session.id, url=session.url, generation=generation
            )
            await db.commit()
            return {"checkout_url": session.url}
            ...
            await db.rollback()
            raise APIException(
                message="Stripe checkout failed", code="PAYMENT_PROVIDER_FAILED", status_code=502
            ) from exc
        except Exception:
            await db.rollback()
            raise
    ...
    async def webhook(self, db: AsyncSession, *, payload: bytes, signature: str) -> dict:
        ...
        try:
            ...
            raise APIException(
                message="Invalid payment webhook signature", code="INVALID_WEBHOOK", status_code=400
            ) from exc
        ...
        if not isinstance(event_id, str) or not event_id:
            raise APIException(message="Payment webhook event ID is missing", status_code=400)
        ...
        try:
            ...
            if not invoice:
                raise APIException(
                    message="Checkout has not been recorded; retry webhook",
                    code="CHECKOUT_NOT_RECORDED",
                    status_code=503,
                )
            metadata = session.get("metadata", {})
            if (
                metadata.get("invoice_id") != invoice.id
                or metadata.get("organization_id") != invoice.organization_id
                or session.get("currency") != invoice.currency.lower()
                or session.get("amount_total")
                != stripe_minor_units(invoice.amount, invoice.currency)
            ):
                raise ConflictError(
                    message="Payment does not match invoice", code="PAYMENT_MISMATCH"
                )
            ...
            payment_methods = session.get("payment_method_types") or []
            payment_method = payment_methods[0] if payment_methods else None
            payment = await self.repository.record_payment(
                db,
                invoice,
                intent_id=intent_id,
