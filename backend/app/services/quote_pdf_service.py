...
def render_quote_pdf(
    *, organization: dict, customer: dict, quote: dict, items: list[dict]
) -> bytes:
    output = BytesIO()
    ...
    story = [
        paragraph("SALES QUOTE", "Title"),
        Spacer(1, 6 * mm),
        paragraph(organization["name"], "Heading2"),
        paragraph(organization.get("address")),
        paragraph(organization.get("email")),
        Spacer(1, 5 * mm),
        ...
        Spacer(1, 5 * mm),
        paragraph("Prepared for", "Heading3"),
    ...
    for item in items:
        rows.append(
            [
                ...
                paragraph(item["quantity"]),
                paragraph(f"{item['unit_price']:.2f}"),
                paragraph(f"{item['discount_percent']:.2f}"),
                paragraph(f"{item['tax_percent']:.2f}"),
                paragraph(f"{item['total']:.2f}"),
            ]
        )
    table = Table(
    ...
    story.extend(
        [
            ...
            Spacer(1, 6 * mm),
            paragraph(
                f"Subtotal: {quote['currency']} {sum((item['subtotal'] for item in items), Decimal(0)):.2f}"
            ),
            paragraph(
                f"Discount: {quote['currency']} {sum((item['discount_total'] for item in items), Decimal(0)):.2f}"
            ),
            paragraph(
                f"Tax: {quote['currency']} {sum((item['tax_total'] for item in items), Decimal(0)):.2f}"
            ),
            paragraph(f"Grand total: {quote['currency']} {quote['total_amount']:.2f}", "Heading2"),
            paragraph(
                "Review and accept this quote using the secure link in your quote email. "
