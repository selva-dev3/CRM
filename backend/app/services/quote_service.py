...
class QuoteService:
    ...
    async def create_quote(
        ...
        if not deal:
            raise NotFoundError(message=f"Deal '{payload.deal_id}' not found")
        if payload.status not in QUOTE_STATUSES:
            raise APIException(
                message=f"Invalid quote status '{payload.status}'.", code="INVALID_QUOTE_STATUS"
            )
        ...
        try:
            ...
            await db.rollback()
            raise APIException(message="Failed to create quote") from exc
    ...
    async def delete_quote(self, db: AsyncSession, *, quote_id: str, organization_id: str) -> None:
        ...
        if not quote:
            raise NotFoundError(message=f"Quote '{quote_id}' not found")
        await self.repository.delete(db, quote)
        try:
            await db.commit()
        except Exception as exc:
            await db.rollback()
            raise APIException(message="Failed to delete quote") from exc

    async def update_quote(
        self, db: AsyncSession, *, quote_id: str, payload: QuoteBase, organization_id: str
    ) -> dict:
        quote = await self.repository.get_scoped(
            db, quote_id=quote_id, organization_id=organization_id
        )
        ...
        quote.deal_id, quote.quote_number, quote.total_amount, quote.status = (
            deal.id,
            payload.quote_number.strip() or quote.quote_number,
        ...
        try:
            ...
            await db.rollback()
            raise APIException(message="Failed to update quote") from exc
        await db.refresh(quote)
        return quote_to_dict(quote)
