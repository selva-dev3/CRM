...
class RoleService:
    ...
    async def _get_default_role_ids(self, db: AsyncSession) -> set:
        try:
            setting = await self.repository.get_setting(db, "default_registration_roles")
            ...
            legacy = await self.repository.get_setting(db, "default_registration_role")
    ...
    async def list_roles(
        ...
        for r in roles:
            ...
            if r.id in default_ids or r.name in default_ids:
                ...
                role_type = "custom"
            result.append(role_to_dict(r, perm_keys, str(getattr(r, "created_at", "2026-08-05"))) | {"type": role_type})
        return result

    # --- Create role ---
    async def create_role(self, db: AsyncSession, payload: RoleCreate) -> dict:
        role = await self.repository.create_role(db, name=payload.name, description=payload.description or "")
        try:
            await db.commit()
            await db.refresh(role)
        except Exception as e:
            await db.rollback()
            raise APIException(status_code=status.HTTP_400_BAD_REQUEST, message=f"Failed to create role: {str(e)}") from e

        saved_permissions = []
        if payload.permissions:
            found_perms = await self.repository.get_permissions_by_keys_or_ids(db, payload.permissions)
            ...
            for key_str in missing_keys:
                if key_str and isinstance(key_str, str):
                    ...
                    new_perm = await self.repository.create_permission(db, data={"key": key_str, "name": name, "category": category, "description": name})
                    await db.flush()
        ...
        return role_to_dict(role, saved_permissions, str(getattr(role, "created_at", datetime.now().isoformat()))) | {"type": "custom"}
    ...
    async def import_permissions_batch(self, db: AsyncSession, payload: list[PermissionCreate]) -> dict:
        try:
            count = 0
            for item in payload:
    ...
    async def set_multiple_default_roles(self, db: AsyncSession, role_ids: list[str]) -> dict:
        try:
            ...
            await self.repository.upsert_setting(db, "default_registration_roles", new_val, "Default registration roles JSON array")
            if role_ids:
                await self.repository.upsert_setting(db, "default_registration_role", role_ids[0], "Legacy default role")
            await db.commit()
            return {"message": f"Successfully updated default registration roles ({len(role_ids)} selected)", "status": "success"}
        except Exception as e:
            await db.rollback()
            raise APIException(status_code=status.HTTP_400_BAD_REQUEST, message=str(e)) from e

    # --- Get default role ---
    async def get_default_role(self, db: AsyncSession) -> dict:
        fallback = {"id": "sys-manager", "name": "manager", "description": "Default role", "permissions": ["dashboard:read", "users:read"], "is_system_role": True, "created_at": "2026-08-05"}
        try:
            setting = await self.repository.get_setting(db, "default_registration_role")
            role_obj = None
            if setting and setting.value:
            ...
            if not role_obj:
                res = await db.execute(select(Role).where(Role.is_system_role == True).limit(1))  # noqa: E712
            ...
            if role_obj:
                ...
                perm_keys = [p.key for p in assigned if p.key] if assigned else ["dashboard:read", "users:read"]
                return role_to_dict(role_obj, perm_keys, str(getattr(role_obj, "created_at", "2026-08-05"))) | {"description": role_obj.description or "Default Registration Role"}
        except Exception:
            return fallback
        return fallback
    ...
    async def get_user_role(self, db: AsyncSession, user_id: str) -> dict:
        ...
        if not role_obj:
            role_obj = await self.repository.get_role(db, user_id)
        if not role_obj:
            role_obj = await self.repository.get_first_role(db)
        if role_obj:
    ...
    # --- Update role ---
    async def update_role(self, db: AsyncSession, role_id: str, payload: RoleUpdate) -> dict:
        r = await self.repository.get_role(db, role_id)
        if not r:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        ...
        try:
            ...
            return role_to_dict(r, payload.permissions if payload.permissions is not None else [], str(getattr(r, "created_at", "2026-08-05")))
        except Exception as e:
            await db.rollback()
            raise APIException(status_code=status.HTTP_400_BAD_REQUEST, message=str(e)) from e

    # --- Delete role ---
    async def delete_role(self, db: AsyncSession, role_id: str) -> dict:
        r = await self.repository.get_role_by_id_or_name(db, role_id)
        if not r:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        default_ids = await self._get_default_role_ids(db)
    ...
    # --- Clone role ---
    async def clone_role(self, db: AsyncSession, role_id: str, new_name: str) -> dict:
        orig = await self.repository.get_role(db, role_id)
        if not orig:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        r = await self.repository.create_role(db, name=new_name, description=f"Cloned from {orig.name}")
        ...
        return role_to_dict(r, [op.key for op in orig_perms] if orig_perms else ["dashboard:read"], str(getattr(r, "created_at", datetime.now().isoformat())))

    # --- Assign permissions to role ---
    async def assign_permissions(self, db: AsyncSession, role_id: str, permissions: list[str]) -> dict:
        role = await self.repository.get_role(db, role_id)
        if not role:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        self._ensure_mutable_role(role)
    ...
    async def remove_permission(self, db: AsyncSession, role_id: str, perm_id: str) -> dict:
        role = await self.repository.get_role(db, role_id)
        if not role:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        self._ensure_mutable_role(role)
    ...
    # --- Get role users ---
    async def get_role_users(self, db: AsyncSession, role_id: str) -> list[dict]:
        r = await self.repository.get_role_by_id_or_name(db, role_id)
        target_role_id = r.id if r else role_id
        target_role_name = r.name if r else role_id
        ...
        matched = list(user_dict.values())
        if matched:
            return [{"id": u.id, "name": u.name, "email": u.email, "role": target_role_name, "created_at": str(getattr(u, "created_at", "2026-08-05"))} for u in matched]
        return [
    ...
    # --- Set default role ---
    async def set_default_role(self, db: AsyncSession, role_id: str) -> dict:
        r = await self.repository.get_role_by_id_or_name(db, role_id)
        if not r:
            raise NotFoundError(message=f"Role '{role_id}' not found")
        setting = await self.repository.get_setting(db, "default_registration_roles")
        ...
        await self.repository.upsert_setting(db, "default_registration_roles", new_val, "Default registration roles JSON array")
        if current_defaults:
            await self.repository.upsert_setting(db, "default_registration_role", current_defaults[0], "Legacy single default role")
        await db.commit()
