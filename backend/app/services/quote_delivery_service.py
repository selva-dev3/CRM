...
def acceptance_token(quote_id: str, delivery_id: str) -> str:
    return hmac.new(settings.SECRET_KEY.encode(),
        f"quote-acceptance:{quote_id}:{delivery_id}".encode(), hashlib.sha256).hexdigest()


class QuoteDeliveryService:
    async def queue(self, db, *, quote_id: str, organization_id: str, recipient_email: str) -> dict:
        try:
            quote = await quote_repository.lock_scoped(db, quote_id=quote_id, organization_id=organization_id)
            if not quote:
                raise NotFoundError(message="Quote not found")
            if quote.delivery_status in {"Pending", "Processing", "Sent"}:
                result = {"message": "Quote delivery already recorded", "status": quote.delivery_status}
                await db.commit()
                return result
            if quote.delivery_status == "Unknown":
                raise APIException(message="Delivery outcome needs provider reconciliation before resending",
                                   code="QUOTE_DELIVERY_UNKNOWN", status_code=409)
            ...
            await quote_repository.queue_delivery(db, quote, delivery_id=delivery_id,
                recipient_email=contact.email, token_hash=hashlib.sha256(token.encode()).hexdigest())
            await db.commit()
            return {"message": "Quote queued for PDF generation and email delivery", "status": "Pending"}
        except Exception:
            await db.rollback()
            raise
    ...
    async def deliver_one(self, session_factory) -> bool:
        ...
        async with session_factory() as db:
            ...
            try:
                ...
                company, contact = await DealRepository().get_sales_customer(db,
                    organization_id=org_id, company_id=quote.company_id, contact_id=quote.contact_id)
                if not organization or not company or not contact or contact.company_id != company.id:
                    raise ValueError("Quote customer is invalid")
                items = await quote_repository.list_items(db, quote_id=quote_id, organization_id=org_id)
                if not items:
                    raise ValueError("Quote items are missing")
                document = {
                    "organization": {"name": organization.name, "address": organization.address,
                                     "email": organization.email},
                    "customer": {"company": company.name, "name": contact.name, "email": quote.recipient_email},
                    "quote": {"quote_number": quote.quote_number, "currency": quote.currency,
                              "expires_at": quote.expires_at.date().isoformat(), "total_amount": quote.total_amount},
                    "items": [{key: getattr(item, key) for key in (
                        "product_name", "quantity", "unit_price", "discount_percent", "tax_percent",
                        "subtotal", "discount_total", "tax_total", "total")} for item in items],
                }
        ...
        try:
            ...
            pdf_key = await asyncio.to_thread(s3_service.upload_file, BytesIO(pdf),
                f"{org_id}/quotes/{quote_id}/{delivery_id}.pdf", "application/pdf")
            ...
            body = (f"<p>Hello {escape(document['customer']['name'])},</p>"
            ...
            email_started = True
            message_id = await asyncio.to_thread(send_tracked_email, to_email=recipient,
                subject=f"Quote {document['quote']['quote_number']}", html_content=body, idempotency_key=delivery_id)
            state = "Sent"
            ...
            logger.warning("Quote delivery failed quote_id=%s error_type=%s", quote_id, type(exc).__name__)

        async with session_factory() as db:
            quote = await quote_repository.lock_scoped(db, quote_id=quote_id, organization_id=org_id)
            if quote and quote.delivery_id == delivery_id and quote.delivery_status == "Processing":
                await quote_repository.delivery_result(db, quote, state=state, pdf_key=pdf_key,
                    message_id=message_id, at=datetime.now(UTC))
                await db.commit()
        return True
