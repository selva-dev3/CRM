...
class Settings(BaseSettings):
    ...
    @property
    def susanoox_model_pool(self) -> list[str]:
        """Return the configured Susanoox fallback order without duplicates."""
        return list(
            dict.fromkeys(
                model.strip() for model in self.SUSANOOX_MODEL_POOL.split(",") if model.strip()
            )
        )
