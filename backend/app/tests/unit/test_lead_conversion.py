...
def conversion_fixture():
    ...
    lead = Lead(id="lead", organization_id="org", title="Sale", company="Acme",
                contact_name="Buyer", email="buyer@example.test", status="Qualified",
                is_archived=False)
...
async def test_conversion_persists_real_links_and_is_repeatable(create_deal):
    ...
    result = await service.convert_lead(db, lead.id, LeadConvertRequest(create_deal=create_deal), user)
    ...
    assert db.add.call_count == (4 if create_deal else 3)
...
async def test_invalid_or_legacy_conversion_is_rejected(lead_status):
    ...
    with pytest.raises(APIException):
        await LeadService(repository=repository).convert_lead(db, lead.id, LeadConvertRequest(), user)
