...
def invitation_membership_repository(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "lock_invitation_organization",
        AsyncMock(return_value=_make_org(max_users=100)),
    )
    monkeypatch.setattr(
...
async def test_list_users_maps_role_names():
    ...
    repo.effective_role_names_for_users = AsyncMock(return_value={user.id: "Sales Manager"})
    ...
    assert as_async_mock(repo.list).await_args_list[-1].kwargs["organization_id"] == "org-1"
...
async def test_create_user_hashes_password(monkeypatch):
    ...
    assert as_async_mock(repo.create).await_args_list[-1].kwargs["data"]["role"] == "role-1"
    assert (
        as_async_mock(repo.create).await_args_list[-1].kwargs["data"]["hashed_password"]
        == EXPECTED_HASHED_VALUE
    )
    assert (
        as_async_mock(repo.create).await_args_list[-1].kwargs["data"]["organization_id"] == "org-1"
    )
...
async def test_create_user_derives_org_from_authenticated_user():
    ...
    db = AsyncMock(spec=AsyncSession)
    role = type(
        "R", (), {"id": "role-1", "name": "Sales Executive", "organization_id": "org-current"}
    )()
    cast(Any, service.role_repository).get_role_by_id_or_name = AsyncMock(return_value=role)
    ...
    assert (
        as_async_mock(organization_repository.get_by_id).await_args_list[-1].args[1]
        == "org-current"
    )
    assert (
        as_async_mock(repo.create).await_args_list[-1].kwargs["data"]["organization_id"]
        == "org-current"
    )
...
async def test_create_user_assigns_system_role():
    ...
    assert require_await(repo.create).kwargs["data"]["role"] == "role-sales-manager"
...
async def test_delete_user_protects_platform_admin():
    ...
    repo.effective_role_names_for_users = AsyncMock(return_value={user.id: "Sales Executive"})
    service = _service_with(repo)
    db = AsyncMock(spec=AsyncSession)
...
async def test_invite_users_uses_current_user_org_and_stores_role_id(monkeypatch):
    ...
    assert (
        as_async_mock(repo.create_invitation).await_args_list[-1].kwargs["data"]["role"] == "role-1"
    )
    assert (
        as_async_mock(repo.create_invitation).await_args_list[-1].kwargs["data"]["organization_id"]
        == "org-1"
    )
...
async def test_invite_users_accepts_normalized_active_organization_status(monkeypatch):
    ...
    assert result["status"] == "success"
    as_async_mock(repo.create_invitation).assert_awaited_once()
...
async def test_invite_users_rejects_inactive_or_disabled_organization(
    ...
    as_async_mock(repo.create_invitation).assert_not_awaited()
...
async def test_invite_users_assigns_sales_manager_when_system_role(monkeypatch):
    ...
    monkeypatch.setattr("app.services.user_service.send_user_invite_email", lambda **kwargs: None)
    payload = UserInviteRequest(users=[{"email": "invite@crm.com"}], role="role-sales-manager")
...
async def test_update_user_rejects_last_admin_demotion():
    ...
    service = _service_with(repo)
    cast(Any, service.role_repository).get_role_by_id_or_name = AsyncMock(return_value=replacement)
    cast(Any, service.role_repository).replace_user_role = AsyncMock()
...
async def test_set_user_quota_persists_and_commits():
    ...
    as_async_mock(repo.upsert_quota).assert_awaited_once_with(
...
async def test_set_user_quota_rejects_cross_org_target():
    ...
    as_async_mock(repo.upsert_quota).assert_not_awaited()
...
async def test_invitation_batch_failure_rolls_back_and_sends_no_email(monkeypatch):
    ...
    repo = UserRepository()
    replace_attr(repo, "create_invitation", AsyncMock(return_value=UserInvitation(id="inv-test")))
    service = _service_with(repo)
    replace_attr(service.organization_repository, "get_by_id", AsyncMock(return_value=_make_org()))
    replace_attr(
        service,
        "_resolve_assignable_role",
        AsyncMock(return_value=type("Role", (), {"id": "role-1", "name": "Sales Executive"})()),
    )
    monkeypatch.setattr(
        OrganizationLifecycleRepository, "email_in_use", AsyncMock(side_effect=[False, True])
    )
    send = Mock()
    monkeypatch.setattr("app.services.user_service.send_user_invite_email", send)
    db = AsyncMock(spec=AsyncSession)
    with pytest.raises(APIException) as error:
        await service.invite_users(
            db,
            UserInviteRequest(
...
async def test_direct_user_creation_enforces_member_limit_before_writes(monkeypatch):
    ...
    repo = UserRepository()
    replace_attr(repo, "create", AsyncMock())
    service = _service_with(repo)
    replace_attr(service.organization_repository, "get_by_id", AsyncMock(return_value=_make_org()))
    monkeypatch.setattr(
