...
@router.get("", response_model=List[ContactResponse], summary="List all contacts", dependencies=[Depends(require_permission("contacts:read"))])
...
@router.get("/starred", response_model=List[ContactResponse], summary="Get starred contacts list", dependencies=[Depends(require_permission("contacts:read"))])
async def get_starred_contacts(db: AsyncSession = Depends(get_db)):
    return await contact_service.get_starred_contacts(db)


@router.post("/merge", response_model=MessageResponse, summary="Merge two contact profiles", dependencies=[Depends(require_permission("contacts:update"))])
...
@router.get("/export/csv", summary="Export contacts as CSV", dependencies=[Depends(require_permission("contacts:export"))])
async def export_contacts_csv():
    return {"download_url": "https://api.crm.com/exports/contacts.csv"}


@router.post("/import/csv", response_model=MessageResponse, summary="Import contacts from CSV", dependencies=[Depends(require_permission("contacts:import"))])
async def import_contacts_csv():
    return {"message": "Import completed successfully", "status": "success"}


@router.post("/bulk-delete", response_model=BulkActionResponse, summary="Bulk delete contacts", dependencies=[Depends(require_permission("contacts:bulk_delete"))])
...
@router.get("/{contact_id}", response_model=ContactResponse, summary="Get contact details by ID", dependencies=[Depends(require_permission("contacts:read"))])
...
@router.put("/{contact_id}", response_model=ContactResponse, summary="Update contact by ID", dependencies=[Depends(require_permission("contacts:update"))])
...
@router.delete("/{contact_id}", response_model=MessageResponse, summary="Delete contact by ID", dependencies=[Depends(require_permission("contacts:delete"))])
...
@router.get("/{contact_id}/deals", response_model=List[DealResponse], summary="List deals linked to contact", dependencies=[Depends(require_permission("contacts:read"))])
async def get_contact_deals(contact_id: str, db: AsyncSession = Depends(get_db)):
    await contact_service.get_contact(db, contact_id)
    return []


@router.get("/{contact_id}/activities", summary="Get activity timeline for contact", dependencies=[Depends(require_permission("contacts:read"))])
async def get_contact_activities(contact_id: str, db: AsyncSession = Depends(get_db)):
    await contact_service.get_contact(db, contact_id)
    return []


@router.post("/{contact_id}/star", response_model=MessageResponse, summary="Star contact", dependencies=[Depends(require_permission("contacts:update"))])
...
@router.post("/{contact_id}/unstar", response_model=MessageResponse, summary="Unstar contact", dependencies=[Depends(require_permission("contacts:update"))])
...
@router.post("/{contact_id}/notes", response_model=NoteResponse, summary="Add note to contact", dependencies=[Depends(require_permission("contacts:create"))])
...
@router.get("/{contact_id}/emails", response_model=List[EmailResponse], summary="List emails linked to contact", dependencies=[Depends(require_permission("contacts:read"))])
async def get_contact_emails(contact_id: str, db: AsyncSession = Depends(get_db)):
    await contact_service.get_contact(db, contact_id)
    return []


@router.get("/{contact_id}/calls", response_model=List[CallLogResponse], summary="List call logs for contact", dependencies=[Depends(require_permission("contacts:read"))])
