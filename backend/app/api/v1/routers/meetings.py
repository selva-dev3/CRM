...
@router.post(
    "/zoom/create-link",
    summary="Generate Zoom meeting room URL",
    dependencies=[Depends(require_permission("meetings:create"))],
)
async def create_zoom_link(topic: str = "CRM Meeting", start_time: str | None = None):
    return await meeting_service.create_zoom_link(topic)


@router.post(
    "/teams/create-link",
    summary="Generate Microsoft Teams meeting URL",
    dependencies=[Depends(require_permission("meetings:create"))],
)
async def create_teams_link(subject: str = "CRM Meeting", start_time: str | None = None):
...
@router.post(
    ...
    response_model=MessageResponse,
    summary="Reschedule meeting time",
    dependencies=[Depends(require_permission("meetings:update"))],
)
