...
async def list_users(
    ...
    total = await user_service.count_users(db, search=search, current_user=current_user)
...
@router.post(
    "/invite",
    response_model=UserInviteBulkResponse,
...
@router.get(
    "/invitations",
    response_model=list[UserInvitationDetailsResponse],
...
@router.get(
    "/invitations/all",
    response_model=list[UserInvitationDetailsResponse],
    summary="List all user invitations (alias)",
    dependencies=[Depends(require_permission("users:read"))],
)
...
@router.get(
    "/export/csv",
    summary="Export users list as CSV file",
...
@router.get(
    "/{user_id}/quota",
    summary="Get user sales quota target",
    dependencies=[Depends(require_permission("users:read"))],
)
async def get_user_quota(
    ...
    return await user_service.get_user_quota(db, user_id, current_user=current_user)


@router.post(
    "/{user_id}/quota",
    response_model=MessageResponse,
    summary="Set user quarterly sales quota target",
    dependencies=[Depends(require_permission("users:update"))],
)
async def set_user_quota(
    ...
    return await user_service.set_user_quota(
        db, user_id=user_id, target_amount=target_amount, current_user=current_user
    )
