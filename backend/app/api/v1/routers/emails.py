...
@router.get("/inbox", summary="Fetch unified inbox email messages", dependencies=[Depends(require_permission("emails:read"))])
async def get_inbox(
    page: int = 1,
    limit: int = 20,
    folder: str = "inbox",
...
@router.get("/drafts", summary="List saved email drafts", dependencies=[Depends(require_permission("emails:read"))])
async def list_drafts(db: AsyncSession = Depends(get_db)):
    return await email_domain_service.list_drafts()


@router.post("/drafts", response_model=MessageResponse, summary="Save draft email message", dependencies=[Depends(require_permission("emails:send"))])
async def save_draft(payload: EmailSendRequest, db: AsyncSession = Depends(get_db)):
    return await email_domain_service.save_draft()


@router.get("/drafts/{draft_id}", summary="Get draft email by ID", dependencies=[Depends(require_permission("emails:read"))])
async def get_draft(draft_id: str, db: AsyncSession = Depends(get_db)):
    return await email_domain_service.get_draft(draft_id)


@router.delete(
    "/drafts/{draft_id}", response_model=MessageResponse, summary="Delete draft email",
...
@router.get("/templates", summary="List email templates", dependencies=[Depends(require_permission("emails:templates"))])
async def list_email_templates(db: AsyncSession = Depends(get_db)):
    return await email_domain_service.list_templates(db)


@router.post(
    "/templates", response_model=MessageResponse, summary="Create new email template",
...
@router.get("/templates/{template_id}", summary="Get email template by ID", dependencies=[Depends(require_permission("emails:templates"))])
...
@router.post("/campaigns/send-bulk", summary="Send bulk email campaign blast to target list", dependencies=[Depends(require_permission("emails:send"))])
...
@router.get("/tracking/{email_id}/status", summary="Get email open & link click analytics", dependencies=[Depends(require_permission("emails:read"))])
...
@router.get("/signatures", summary="List user email signatures", dependencies=[Depends(require_permission("emails:read"))])
...
@router.get("/threads/{thread_id}", summary="Get full email conversation thread", dependencies=[Depends(require_permission("emails:read"))])
