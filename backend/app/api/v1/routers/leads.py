...
@router.post("/bulk/archive", response_model=BulkActionResponse, summary="Bulk archive leads", dependencies=[Depends(require_permission("leads:bulk_update"))])
...
@router.post("", response_model=LeadResponse, status_code=status.HTTP_201_CREATED, summary="Create a new lead", dependencies=[Depends(require_permission("leads:create"))])
...
@router.get("/sources", summary="Get all lead sources", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_sources():
    return LEAD_SOURCES


@router.post("/sources", response_model=MessageResponse, summary="Create new lead source", dependencies=[Depends(require_permission("leads:create"))])
async def create_lead_source(source_name: str):
    return {"message": f"Lead source {source_name} created", "status": "success"}


@router.get("/statuses", summary="Get all lead status stages", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_statuses():
    return LEAD_STATUSES


@router.post("/statuses", response_model=MessageResponse, summary="Create new lead status stage", dependencies=[Depends(require_permission("leads:create"))])
async def create_lead_status(status_name: str):
    return {"message": f"Lead status {status_name} created", "status": "success"}


@router.post("/check-duplicates", summary="Check duplicate lead by email or phone", dependencies=[Depends(require_permission("leads:read"))])
...
@router.get("/analytics/by-source", summary="Get lead conversion analytics by source", dependencies=[Depends(require_permission("leads:read"))])
async def lead_analytics_by_source():
    return []


@router.get("/export/csv", summary="Export leads list as CSV", dependencies=[Depends(require_permission("leads:export"))])
async def export_leads_csv():
    return {"download_url": "https://api.crm.com/exports/leads.csv"}


@router.post("/import/csv", response_model=MessageResponse, summary="Import leads from CSV file", dependencies=[Depends(require_permission("leads:import"))])
async def import_leads_csv():
    return {"message": "Import completed successfully", "status": "success"}


@router.post("/bulk-update-status", response_model=BulkActionResponse, summary="Bulk update lead status", dependencies=[Depends(require_permission("leads:bulk_update"))])
...
@router.get("/{lead_id}", response_model=LeadResponse, summary="Get lead details by ID", dependencies=[Depends(require_permission("leads:read"))])
...
@router.post("/{lead_id}/convert", summary="Convert lead to Deal, Contact, and Company", dependencies=[Depends(require_permission("leads:convert"))])
...
@router.post("/{lead_id}/assign", response_model=MessageResponse, summary="Assign lead to specific sales rep", dependencies=[Depends(require_permission("leads:assign"))])
...
@router.post("/{lead_id}/score", summary="Recalculate AI score for lead", dependencies=[Depends(require_permission("leads:update"))])
...
@router.get("/{lead_id}/timeline", summary="Get activity timeline for lead", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_timeline(lead_id: str, db: AsyncSession = Depends(get_db)):
    return await lead_service.get_timeline(db, lead_id)


@router.get("/{lead_id}/notes", response_model=list[NoteResponse], summary="List notes attached to lead", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_notes(lead_id: str, db: AsyncSession = Depends(get_db)):
    return await lead_service.get_notes(db, lead_id)


@router.post("/{lead_id}/notes", response_model=NoteResponse, summary="Add note to lead", dependencies=[Depends(require_permission("leads:create"))])
...
@router.get("/{lead_id}/tasks", response_model=list[TaskResponse], summary="List tasks assigned to lead", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_tasks(lead_id: str, db: AsyncSession = Depends(get_db)):
    return await lead_service.get_tasks(db, lead_id)


@router.post("/{lead_id}/tasks", response_model=TaskResponse, summary="Create task for lead", dependencies=[Depends(require_permission("leads:create"))])
...
@router.get("/{lead_id}/emails", response_model=list[EmailResponse], summary="List emails exchanged with lead", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_emails(lead_id: str, db: AsyncSession = Depends(get_db)):
    return await lead_service.get_emails(db, lead_id)


@router.post("/{lead_id}/emails/send", response_model=EmailResponse, summary="Send email to lead", dependencies=[Depends(require_permission("emails:send"))])
...
@router.get("/{lead_id}/calls", response_model=list[CallLogResponse], summary="List call logs for lead", dependencies=[Depends(require_permission("leads:read"))])
async def get_lead_calls(lead_id: str, db: AsyncSession = Depends(get_db)):
    return await lead_service.get_calls(db, lead_id)


@router.post("/{lead_id}/calls", response_model=CallLogResponse, summary="Log call with lead", dependencies=[Depends(require_permission("leads:create"))])
...
@router.get("/{lead_id}/documents", response_model=list[DocumentResponse], summary="List documents attached to lead", dependencies=[Depends(require_permission("leads:read"))])
...
@router.post("/{lead_id}/unarchive", response_model=MessageResponse, summary="Unarchive lead", dependencies=[Depends(require_permission("leads:update"))])
