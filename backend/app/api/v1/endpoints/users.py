...
async def create_user(payload: UserCreate):
    return {"id": "usr-3", "name": payload.name, "email": payload.email, "role": payload.role, "organization_id": payload.organization_id, "is_active": True, "created_at": "2026-08-02"}
...
async def update_my_profile(payload: UserProfileUpdate):
...
async def invite_users(payload: UserInviteRequest):
...
async def assign_user_team(user_id: str, team_id: str):
...
async def remove_user_team(user_id: str, team_id: str):
...
async def get_user_effective_permissions(user_id: str):
...
async def set_user_quota(user_id: str, target_amount: float):
...
async def get_user_scorecard(user_id: str):
