...
async def create_custom_report(name: str, filters: Dict[str, Any]):
...
async def run_custom_report(report_id: str):
...
async def export_report_pdf(report_type: str):
...
async def export_report_csv(report_type: str):
    return {"csv_url": f"https://api.crm.com/exports/reports_{report_type}.csv"}

@router.post("/schedule", response_model=MessageResponse, summary="Schedule recurring automated email delivery of report")
async def schedule_report_email(report_type: str, email: str, frequency: str = "Weekly"):
