...
class DashboardKPIs(BaseModel):
    ...
    quote_value: float = Field(ge=0)
    quote_conversion_percentage: float = Field(ge=0, le=100)
    ...
    partially_paid_invoice_count: int = Field(ge=0)
    paid_invoice_count: int = Field(ge=0)
    revenue: float = Field(ge=0)
    collection_rate_percentage: float = Field(ge=0, le=100)
